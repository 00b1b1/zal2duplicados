...
const ProcessedFilesListEnhanced = () => {
  ...
  const [featuredDays, setFeaturedDays] = useState<FeaturedDay[]>([]);
  ...
  const handleRefresh = () => {
    ...
    toast({
      title: "Actualizado",
      description: "La lista de archivos ha sido actualizada.",
    });
  };
  ...
  const searchFiles = (files: ProcessedFile[]) => {
    ...
    return files.filter(file => 
      file.orders.some(order => 
        order.order.toLowerCase().includes(lowerQuery) || 
        order.supplier.toLowerCase().includes(lowerQuery)
      )
    );
  };

  const filteredFiles = searchFiles(files)
  ...
  return (
    <Card className="w-full shadow-md">
      <CardHeader className="bg-dhl-yellow flex flex-row items-center justify-between">
        <CardTitle className="text-dhl-red">Archivos Guardados</CardTitle>
        <Button 
          variant="ghost" 
          size="sm" 
      ...
      <CardContent className="p-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <Search size={16} />
              <span className="text-sm font-semibold">Buscar pedido o proveedor:</span>
            </div>
            <Input
              placeholder="Ej: 12345 o Nombre Proveedor"
              value={searchQuery}
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Día destacado</TableHead>
                  <TableHead className="text-right">Acciones</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {filteredFiles.map((file) => {
                  const featuredDay = getFeaturedDay(file.fileName);
                  return (
                    <TableRow key={file.id}>
                      ...
                      <TableCell>
                        {featuredDay ? (
                          <Badge className="bg-dhl-yellow text-black hover:bg-dhl-yellow">
                            {featuredDay.day} ({featuredDay.day_of_week})
                          </Badge>
                        ) : (
                          <span className="text-gray-400 text-sm">No destacado</span>
                        )}
                      </TableCell>
...
export default ProcessedFilesListEnhanced;
